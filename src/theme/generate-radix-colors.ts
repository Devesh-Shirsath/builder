// @ts-nocheck -- Vendored code, kept byte-comparable to upstream.
// The installed colorjs.io types declare `Color.coords` as `(number | null)[]`,
// while the version Radix pins declares `number[]`. That produces ~20 strict-null
// errors here on code that is correct at runtime (verified by lib/palette.test.mts).
// Silencing the file beats rewriting upstream's arithmetic to satisfy typings.
//
// Vendored from radix-ui/website (components/generate-radix-colors.tsx).
// This is the algorithm behind https://www.radix-ui.com/colors/custom.
//
// Two NaN guards were added, each marked "LOCAL DEVIATION" inline. Everything
// else is upstream, untouched. Don't refactor this file — semantic mapping
// belongs in lib/palette.ts, and staying close to upstream keeps it diffable.
...
function getScaleFromColor(
	...
	// LOCAL DEVIATION (guard 1 of 2) — upstream computes `Math.max(0, tanC1 / tanC2) * 0.5`.
	// If the source sits (near) exactly on one of the base scales, b -> 0 and cosA -> ±Infinity,
	// so acos() yields NaN and the NaN propagates through Color.mix into every channel.
	// A degenerate triangle means "don't mix in scale B", i.e. ratio 0.
	const rawRatio = Math.max(0, tanC1 / tanC2) * 0.5;
	...
	// LOCAL DEVIATION (guard 2 of 2) — Radix's `gray` scale is fully achromatic, so when it wins
	// the nearest-scale search baseColor.coords[1] is 0 and this ratio is Infinity. The next line
	// then evaluates `0 * Infinity` = NaN for every step and the whole scale serialises as
	// "#NaNNaNNaN". A zero-chroma base has no chroma profile to rescale, and the right answer for
	// it is a true neutral, so pin the ratio to 0.
	const ratioC = baseColor.coords[1] === 0 ? 0 : source.coords[1] / baseColor.coords[1];
