...
function FieldShell({
  ...
  const active = useStore((s) => (path ? s.selection?.path === path : false))
  const ref = useRef<HTMLDivElement>(null)

  /**
   * The mirror of canvas -> panel: touching a control here selects the element
   * it drives, so the canvas outlines it and scrolls it into view. Guarded on
   * `active` so re-selecting the same field can't loop.
   */
  const selectOnTouch = () => {
    if (!sectionId || !path || active) return
    actions.select({ sectionId, path })
  }

  useEffect(() => {
    if (!active || !ref.current) return
    ref.current.scrollIntoView({ block: 'nearest', behavior: 'smooth' })
    // Never yank focus away from in-place editing on the canvas.
    const editingOnCanvas = (document.activeElement as HTMLElement | null)?.isContentEditable
    if (autofocusable && !editingOnCanvas) {
      const el = ref.current.querySelector('input, textarea') as HTMLInputElement | null
      if (el && el.type !== 'file' && el.type !== 'color') {
        el.focus()
        el.select?.()
      }
    }
  }, [active, autofocusable])
...
function ToggleControl({
  ...
  return (
    <div className={`fgroup ${value ? 'on' : 'off'}`} data-path={path}>
        ...
        {items.map((item, i) => {
          ...
          return (
            <div
              ...
              onDragEnd={endDrag}
            >
              <div className="list-head" onClick={() => setOpen(isOpen ? null : i)}>
                <span
